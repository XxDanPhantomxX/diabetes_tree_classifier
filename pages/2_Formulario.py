...
with col1:
    urin_val = st.selectbox("¿Orina con frecuencia? (Urinating often)", ["Yes", "No"])
    weight_val = st.selectbox("¿Pérdida de peso? (Weight Loss)", ["Yes", "No"])
with col2:
    slow_val = st.selectbox("¿Cicatrización lenta? (Slow Healing)", ["Yes", "No"])
    fat_val = st.selectbox("¿Fatiga extrema? (Extreme Fatigue)", ["Yes", "No"])
