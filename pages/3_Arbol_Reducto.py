from Diabetes import df_diabetes, clfz, df_encoded, featurez_columns, X1_train, X1_test, y1_train, y1_test
...
st.write("## ¿Qué es un Reducto?")
st.write("""
Un reducto es una técnica utilizada en el análisis de datos que permite simplificar un conjunto de datos al eliminar características irrelevantes o redundantes. Esto se hace para mejorar la eficiencia de los modelos de aprendizaje automático y facilitar la interpretación de los resultados.
""")
...
report_train = classification_report(y1_train, y_pred_train, target_names=["Negative", "Positive"], output_dict=True)
report_test = classification_report(y1_test, y_pred_test, target_names=["Negative", "Positive"], output_dict=True)

# Preparar y evaluar sobre el conjunto completo
X_full = df_encoded[featurez_columns]
y_full = df_encoded['Result'].replace({'Positive': 1, 'Negative': 0})
y_pred_full = clfz.predict(X_full)
